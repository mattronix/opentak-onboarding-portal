...
function MeshtasticList() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => alert(err.response?.data?.error || 'Failed to delete config'),
  });
                    ...
                    <div className="table-actions">
                      <button className="btn btn-sm btn-secondary" onClick={async () => {
                        ...
                        try {
                          ...
                          alert('Failed to load config details: ' + (err.response?.data?.error || err.message));
                        }
                      }}>Edit</button>
                      <button className="btn btn-sm btn-danger" onClick={() => {if (window.confirm(`Delete "${config.name}"?`)) deleteMutation.mutate(config.id);}}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
